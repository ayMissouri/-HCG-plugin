...
public final class DecayCommand implements CommandExecutor, TabCompleter {
    ...
    public DecayCommand(HCGPlugin plugin, DecayManager decayManager) {
        this.plugin = plugin;
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "status" -> {
                ...
                Messages.send(sender, "healthdecay.status-health",
                        "current", DecayManager.formatHearts(decayManager.currentMaxHp()),
                        "floor", DecayManager.formatHearts(decayManager.minimumHp()));
                Messages.send(sender, "healthdecay.status-rate",
                        "amount", DecayManager.formatHearts(decayManager.decayAmountHp()),
    ...
    private void turnOn(CommandSender sender) {
        ...
        Messages.broadcastOps("healthdecay.start-broadcast",
                "hearts", DecayManager.formatHearts(decayManager.minimumHp()));
    }
    ...
    private void showMenu(CommandSender sender) {
        ...
        sender.sendMessage(Component.text("Max health: ", NamedTextColor.GRAY)
                .append(Component.text(
                        DecayManager.formatHearts(decayManager.currentMaxHp()) + " / "
                                + DecayManager.formatHearts(decayManager.maximumHp()) + " hearts",
                        NamedTextColor.YELLOW))
                .append(Component.text(" (floor " + DecayManager.formatHearts(decayManager.minimumHp()) + ")",
